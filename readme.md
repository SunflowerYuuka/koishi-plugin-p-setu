...
- **AI作品控制**：默认允许搜索AI图，支持指令一键**排除AI作品**（含客户端二次拦截检测）
...
2. **AI作品拦截**：
* 默认情况：允许搜索到AI作品。
* 使用 `-e` 参数：请求 API 时添加排除参数。
* **二次检测**：如果 API 在排除模式下依然返回了 AI 标记为 true 的图片，插件将在客户端进行二次拦截并提示“哎呀，带这些标签都是AI图呢...”。
...
2. **API说明**：
* 本插件使用 Lolicon API V2，部分冷门标签组合可能无法搜到非AI图，此时会触发客户端拦截提示，请尝试减少标签或更换标签。
